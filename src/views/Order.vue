    ...
    <main class="w-10/12 flex-1 bg-orange-50 p-8 mt-16">
      ...
      <div v-if="loading">
        <img
          ...
          alt="Loading..."
          class="mx-auto w-20 my-20 py-56"
        />
      </div>
      <div v-else>
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            ...
                            class="hover:cursor-pointer absolute border border-gray-400 z-10 bg-white rounded-lg py-2 w-40"
                          >
                            <p
                              @click="updatePayment(order.order_id)"
                              :value="paid"
                              class="block px-6 text-black hover:text-green-500 font-semibold"
                            >
                              Paid
                            </p>
                          </div>
                        ...
                        >
                          ...
                          <div v-if="order.status === 'paid'">
                            <img
                              ...
                              v-on:click="getOrderDetail(order.order_id)"
                              src="../assets/list.png"
                              alt="detail"
                              class="w-6 h-6"
                            />
                          ...
                          <img
                            @click="showConfirmDialog(order.order_id)"
        ...
        <div v-if="showDialog" class="fixed inset-0 z-50">
          ...
          >
            ...
            <div class="flex justify-end mt-4">
              <button
                @click="showDialog = false"
                class="bg-gray-300 py-1 px-10 text-gray-700 rounded-sm mr-4"
              >
                Cancel
              </button>
              <button
                @click="deleteData(order_id)"
                class="bg-primaryRed hover:bg-hoverPrimaryRed py-1 px-10 text-white rounded-sm"
              >
                Delete
              </button>
            </div>
      ...
      <div v-if="showOrderDetailModal" class="fixed inset-0 z-50">
        ...
        ></div>
        <div
          class="top-10 relative pt-6 pb-8 px-6 mx-auto my-auto bg-white rounded-lg w-full h-full max-w-5xl md:h-auto"
        >
          <!-- <p>{{orderDetail}}</p> -->
          <table class="min-w-full">
            <thead class="bg-transparent border-b">
            ...
            <tbody>
              <tr
                v-for="(dt, i) in orderDetail"
                :key="i"
                class="bg-transparent border-b transition duration-300 text-center ease-in-out hover:bg-gray-100"
              >
                <td
                  class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
                >
                ...
                <td
                  class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap"
                >
                  Rp{{ dt.price * dt.quantity }}
                </td>
              </tr>
            </tbody>
          </table>
          <p class="mt-4 font-bold text-orange-500">
            Sub total: Rp{{ totalPrice }}
          </p>
          <button
            @click="goToInvoice(this.order_id)"
            class="border border-orange-500 py-1 px-10 text-orange-500 rounded-lg w-full hover:bg-orange-500 hover:text-white mt-4"
          >
            Print Invoice
          </button>
        </div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50">
        ...
        ></div>
        <div
          class="top-10 relative pt-6 pb-8 px-6 mx-auto my-auto bg-white rounded-lg w-full h-full max-w-3xl md:h-auto"
        >
          ...
          <div class="overflow-y-scroll h-[560px]">
            ...
            <div v-else>
              <div class="px-4">
                <!-- <div class="hidden">
                  <label for="staff" class="block mb-2 text-sm font-medium"
                    >Staff Name</label
                  >
                  <select
                    v-model="user_id"
                ...
                <div class="flex justify-between items-center mt-10">
                  ...
                  <button
                    @click="addMenu"
                    class="bg-orange-500 text-xl px-2 rounded-sm text-white hover:bg-orange-600"
                  >
                    +
                  </button>
                </div>
                ...
                <div v-for="(detail, counter) in detailMenu" :key="counter">
                  ...
                  <div class="flex justify-between items-center">
                    ...
                    <button
                      @click="deleteMenu(counter)"
                      class="text-xl px-2 text-white rounded-sm bg-red-500 hover:bg-red-600"
                    >
                      -
                    </button>
                  </div>
                  <div class="flex justify-between items-center">
                    <div>
                      <label
                        for="menu_id"
                        class="block mb-2 text-sm font-medium"
                        >Menu Name - Price</label
                      >
                      <select
                        v-model="detail.menu_id"
                        class="mb-4 border text-sm rounded-lg block w-full p-2.5"
                      >
                ...
                <button
                  v-on:click="saveData()"
                  class="text-sm mt-4 mx-auto text-orange-500 border border-orange-500 py-2 w-full rounded-lg hover:bg-orange-500 hover:text-white"
                >
                  {{ action }} Order
                </button>
              </div>
...
export default {
  ...
  watch: {
    searchKey: _.debounce(function (newVal) {
      if (newVal.length > 0) {
        ...
        axios
          .get(this.baseUrl + "/order/search/" + newVal, { headers })
          .then((response) => {
    ...
    date: _.debounce(function (newVal) {
      if (newVal !== null) {
        ...
        axios
          .get(this.baseUrl + "/order/searchbydate/" + newVal, { headers })
          .then((response) => {
            this.orders = response.data.data;
          })
          .catch((error) => {
    ...
    month: _.debounce(function (newVal) {
      if (newVal !== null) {
        ...
        axios
          .get(this.baseUrl + "/order/searchbymonth/" + newVal, { headers })
          .then((response) => {
            this.orders = response.data.data;
          })
          .catch((error) => {
  ...
  methods: {
    ...
    getOrderDetail(order_id) {
      let headers = authHeader();
      axios
        .get(this.baseUrl + "/orderdetail/" + order_id, { headers })
        .then((response) => {
          console.log("halo");
    ...
    deleteData() {
      let headers = authHeader();
      axios
        .delete(this.baseUrl + "/order/" + this.order_id, { headers })
        .then((response) => {
    ...
    getData() {
      ...
      if (this.userRole === "cashier") {
        url =
          this.baseUrl + "/order/" + this.$store.state.auth.user.user.user_id;
      } else {
        url = this.baseUrl + "/order";
      }
    ...
    getBasicData() {
      let headers = authHeader();
      axios
        .get(this.baseUrl + "/user/cashier", { headers })
        .then((response) => {
      ...
      axios
        ...
        .then((response) => {
          this.availTables =
            this.action === "Add" ? response.data : response.data.data;
          this.loading = false;
        })
        .catch((error) => {
          this.createAlert(error.response.data.message, "danger", 3000);
    ...
    getDataModal() {
      this.showModal = true;
      (this.action = "Add"), this.getBasicData();
    },
    ...
    getEditModal(order) {
      ...
      axios
        .get(this.baseUrl + "/detailmenu/" + this.order_id, { headers })
        .then((response) => {
    ...
    saveData() {
      ...
      if (this.action === "Add") {
        axios
          .post(this.baseUrl + "/order", form, { headers })
          .then((response) => {
        ...
        axios
          .put(this.baseUrl + "/order/" + this.order_id, form, { headers })
          .then((response) => {
            this.showModal = false;
            this.createAlert(response.data.message, "success", 1500);
            this.getData();
    ...
    updatePayment($order_id) {
      ...
      axios
        .put(this.baseUrl + "/order/" + $order_id + "/status", form, {
          headers,
        })
        .then((response) => {
