  ...
  <div class="flex bg-orange-50 w-full">
    ...
    <main class="w-10/12 flex-1 bg-orange-50 p-8 mt-16">
      ...
      <div class="py-8 flex justify-between items-center">
        <div class="relative">
          ...
          <input
            ...
            v-model="searchKey"
            type="text"
            class="pl-12 py-2 rounded-3xl text-gray-500 bg-white border placeholder:font-light placeholder:text-sm border-orange-300 focus:outline-none focus:bg-white"
            placeholder="Search..."
          />
        </div>
        <button
          ...
          class="border px-10 py-1 text-sm rounded-2xl hover:bg-orange-300 hover:text-white"
        >
          All
        </button>
        <button
          v-on:click="getDataFood()"
        ...
        <button
          ...
          class="border px-10 py-1 text-sm rounded-2xl hover:bg-orange-300 hover:text-white mr-96"
        >
          Drink
        </button>
        <button
          v-if="userRole === 'admin'"
          class="bg-orange-400 px-10 py-2 text-white text-sm rounded-2xl hover:bg-orange-500"
      ...
      <div v-else>
        <div
          v-auto-animate
          class="flex flex-wrap justify-center items-start gap-x-9"
        >
          <div
            v-for="(menu, i) in menus"
            :key="i"
            @click="goToDetail(menu.menu_id)"
          >
            <MenuCard
              v-bind:img="
      ...
      <div v-if="showModal" class="fixed inset-0 z-50">
        ...
        <!-- buat bg menghitam-->
        <div
          class="top-10 relative pt-6 pb-8 px-6 mx-auto my-auto bg-white rounded-lg w-full h-full max-w-xl md:h-auto"
        >
          ...
          <div>
            ...
            <textarea
              v-model="description"
              rows="4"
              class="mb-4 border -sm rounded-lg block w-full p-2.5 text-sm"
              placeholder="ex: Grilled Potato with tomato sauce and side dish..."
              required
            ></textarea>
          </div>
          <div>
          ...
          <button
            v-on:click="addData()"
            class="text-sm mt-4 mx-auto text-white bg-orange-500 py-2 w-full rounded-sm hover:bg-orange-600"
          >
            Add Menu
          </button>
        </div>
