    ...
    <main class="flex-1 bg-orange-50 p-8 mt-16">
      ...
      <div v-else>
        <div class="flex justify-center items-start mt-8 gap-x-10">
          <div class="w-1/3">
            <img
              :src="'http://127.0.0.1:8000/images/' + menu.menu_image_name"
              alt="menu-img"
              class="rounded-xl"
            />
          </div>
          <div class="w-2/3">
            <h1 class="text-4xl text-blue-300 font-bold">
              {{ menu.menu_name }}
            </h1>
            ...
            <div class="flex justify-start gap-x-7 my-6">
              ...
              <button
                ...
                v-on:click="editImage(menu)"
              >
                Edit Image
              </button>
              <button
                @click="showConfirmDialog = true"
                class="border border-orange-500 font-light hover:bg-orange-500 text-orange-500 hover:text-white text-sm px-8 py-1 rounded-full"
              >
                Delete Menu
              </button>
            </div>
...
export default {
  ...
  methods: {
    deleteItem() {
      ...
      axios
        .delete(this.baseUrl + "menu/" + this.$route.params.id, { headers })
        .then((response) => {
    ...
    saveData() {
      ...
      axios
        .put(this.baseUrl + "menu/" + this.menu_id, form, { headers })
        .then((response) => {
          this.menu = response.data.data;
          this.getData();
