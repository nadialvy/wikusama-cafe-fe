    ...
    <div v-else class="flex justify-between items-center">
      <div class="w-1/2 px-44">
        <div>
          ...
          <p class="text-gray-400 font-light text-center">
            Sign in to stay connected.
          </p>
          <div class="mt-8">
            <label
              for="username"
              class="block text-gray-400 mb-2 text-sm font-light"
              >Username</label
            >
            <input
              type="text"
  ...
  methods: {
    ...
    handleLogin() {
      ...
      this.$store.dispatch("auth/login", form).then(
        () => {
          if (
            this.$store.state.auth.user.user.role === "cashier" ||
            this.$store.state.auth.user.user.role === "admin"
          ) {
