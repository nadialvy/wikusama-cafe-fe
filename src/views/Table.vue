    ...
    <main class="w-10/12 flex-1 bg-orange-50 p-8 mt-20">
      <div class="flex justify-between items-center">
        <div class="flex justify-start items-center">
          <div
            class="border-2 border-orange-300 text-blue-300 font-semibold text-5xl w-50 rounded-sm px-4 py-2"
          ></div>
          <p class="ml-4 text-blue-300 mr-10">Available table</p>
          <div
            class="border-2 border-orange-300 bg-orange-200 text-blue-300 text-opacity-50 font-semibold text-5xl w-50 rounded-sm px-4 py-2"
          ></div>
        ...
        <div>
          <button
            v-if="userRole === 'admin'"
            class="bg-orange-400 px-10 py-2 text-white text-sm rounded-2xl hover:bg-orange-500 hover:text-white"
      ...
      <div v-else>
        <div
          v-auto-animate
          class="flex flex-wrap justify-center items-center gap-x-10 mt-10"
        >
          <div v-for="(table, i) in tables" :key="i">
            <TableComponent
              v-bind:isAva="table.is_available"
              v-bind:tableNumber="table.table_number"
            />
          </div>
        </div>
      </div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50">
        ...
        >
          ...
          <button
            v-on:click="addData()"
            class="border border-orange-500 hover:bg-orange-500 hover:text-white py-1 px-10 text-orange-500 rounded-lg w-full"
          >
            Add Table
          </button>
        </div>
...
export default {
  ...
  watch: {
    searchKey: _.debounce(function (newVal) {
      if (newVal.length > 0) {
        ...
        axios
          .get(this.baseUrl + "table/search/" + this.searchKey, { headers })
          .then((response) => {
            this.notFound = false;
  ...
  methods: {
    ...
    addData() {
      ...
      axios
        .post(this.baseUrl + "table", form, { headers })
        .then((response) => {
          this.getData();
