...
<template>
    <div>
        <div v-if="isAva == 'true' " class=" border-4 border-orange-300 w-50 rounded-lg p-8">
            <p class="text-center text-blue-300 font-semibold text-3xl truncate">{{ tableNumber }}</p>
        </div>
        ...
        <div>
            <div v-if="showConfirmDialog" class="fixed inset-0 z-50">
                ...
                <div class="top-80 relative pt-6 pb-8 px-6 mx-auto my-auto bg-white rounded-lg w-full h-full max-w-xl md:h-auto">
                    <p class="text-base font-medium">Are you sure you want to delete table number {{ tableNumber }}?</p>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50">
            ...
            <div class="top-60 relative pt-6 pb-8 px-6 mx-auto my-auto bg-white rounded-lg w-full h-full max-w-xl md:h-auto">
                ...
                <div>
                    ...
                    <input @keyup.enter="saveData()" type="number" v-model="table_number" class="mb-4 border text-sm rounded-lg block w-full p-2.5" placeholder="ex: 104, note that each table must have an unique number" required>
                </div>
                <button v-on:click="saveData()" class="text-sm mt-4 mx-auto text-orange-500 border border-orange-500 py-2 hover:bg-orange-500 hover:text-white w-full rounded-sm hover:bg-hoverPrimaryYellow">Edit Table</button>
            </div>
        </div>
    </div>
</template>
          ...
          editData(tableNumber){
            ...
            saveData(){
              let form = {
                ...
                axios.put(this.baseUrl + this.tableNumber, form, { headers }).then((response) => {
            ...
            deleteItem(){
                let headers = authHeader();
                axios.delete(this.baseUrl + this.tableNumber, { headers }).then((response) => {
                    this.createAlert(response.data.message, 'success', 1500)
                    setTimeout(() => {
                        this.$router.push('/table').then(() => {
                            location.reload();
